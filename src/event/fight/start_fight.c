...
int			prepare_fight(t_prog *prog, t_npc *npc)
{
  ...
  prog->fight->fireball =
  load_image("ressources/sprites/fireballsprite.png", &prog->ptr_list);
  ...
  prog->fight->player->damage =
      get_player_damage(prog->fight->player, prog);
  prog->fight->player->magic_damage =
      get_player_magic_damage(prog->fight->player, prog);
  ...
  if (prog->fight->fetch.x < prog->fight->fireball->width + 83)
    {
      ...
      place_image(prog->fight->pos, prog->fight->fetch,
		  prog->fight->fireball, prog->pix);
      return (1);
    }
  else
    {
