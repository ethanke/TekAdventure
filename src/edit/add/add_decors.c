...
** Last update Sun Apr 17 04:48:33 2016 Gaëtan Léandre
*/
...
void			add_decors(t_ini *ini)
{
  ...
  if ((dec->decors_breakable = xmalloc(sizeof(t_breakable), &ini->ptr_list)) == NULL)
    return;
  if ((dec->decors_breakable->is_breakable = get_decors_collect("Is your decors breakable? (yes or no)", ini)) == -1)
    return;
  if (dec->->decors_breakable->is_breakable == 0)
  ...
  else
    {
      ...
      if ((dec->decors_breakable->lootable = get_decors_collect("Is your decors lootable? (yes or no)", ini)) == -1)
	return;
