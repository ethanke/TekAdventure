...
** Last update Sun Apr 17 06:34:14 2016 Philippe Lefevre
*/
				     ...
				     t_ptr_list **ptr_list, char *name)
{
  ...
  ground = NULL;
  if ((scene = parsing_ground_setting(scene, ini, name)) == NULL)
  return (NULL);
  ...
  scene->ground = ground;
  if ((scene = parsing_ground_grid(scene, ini, name, ptr_list)) == NULL)
    return (NULL);
