					    ...
					    t_ptr_list **ptr_list)
{
  ...
  if ((str = (char *)bunny_ini_get_field(ini, "object",
    ...
    return (my_puterror_object("Error: object:object_is_equipable field ",
			       id, " not found\n"));
  if (((object->is_equipable = my_getnbr(str)) < 0)
      && (object->is_equipable > 1))
    return (my_puterror_object("Error: object:object_sprite_id field ",
			       id, " must be 0 or 1\n"));
  if ((str = (char *)bunny_ini_get_field(ini, "object",
					 "object_equipable_slot", id)) == NULL)
...
t_object		*load_object(t_bunny_ini *ini, t_ptr_list **ptr_list)
{
  ...
  if ((str = (char *)bunny_ini_get_field(ini, "object",
					 "object_count", 0)) == NULL)
    return (my_puterror_object("Error: balise object or object:object_count ",
			       -1, "field not found"));
